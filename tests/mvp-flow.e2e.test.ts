...
describe("MVP flow (listing -> offer -> accept -> pipeline)", () => {
  ...
  it("runs end-to-end stub pipeline", async () => {
    ...
    const sellerReg = await request(app).post("/auth/register").send({
    ...
    const buyerReg = await request(app).post("/auth/register").send({
    ...
    const offerId = offerRes.body.offer.id as string;
    ...
    const pipelineBuyer = await request(app)
    ...
    const listingAfter = await request(app).get(`/listings/${listingId}`);
    ...
    const pipelineFunded = await request(app)
      .get(`/offers/${offerId}/pipeline`)
      .set("Authorization", `Bearer ${buyerReg.body.token}`);
