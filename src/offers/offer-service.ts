...
export class OfferService {
  ...
  async createOffer(buyerUserId: string, listingId: string, payload: unknown) {
    ...
    return this.prisma.offer.create({
      data: {
        listingId,
        buyerUserId,
        offerPrice: new Prisma.Decimal(input.offerPrice),
        earnestMoney: new Prisma.Decimal(earnest),
        status: "SUBMITTED",
      },
  ...
  async acceptOffer(sellerUserId: string, offerId: string) {
  ...
  async getPipelineForUser(offerId: string, userId: string) {
    const offer = await this.prisma.offer.findUnique({
      where: { id: offerId },
      include: {
  ...
  private buildContractDraft(input: {
    buyerName: string;
    sellerName: string;
