...
exports.computeValuation = void 0;
/**
 ...
 */
const computeValuation = (input) => {
    const price = input.askingPrice;
    const area = input.areaM2 ?? 0;
    const impliedMonthlyRent = input.estimatedMonthlyRent ?? Math.round((price * 0.005 + Number.EPSILON) * 100) / 100;
    const annualRent = impliedMonthlyRent * 12;
    const capRate = price > 0 ? (annualRent / price) * 100 : 0;
    const irrEstimate = capRate * 0.92;
    const ceilingPerM2 = input.regionalPriceCeilingPerM2 ??
        (area > 0 ? (price * 1.12) / area : (price * 1.12) / Math.max(area, 1));
    const regionalCeiling = area > 0 ? ceilingPerM2 * area : price * 1.12;
    const discountVsCeiling = regionalCeiling > 0 ? ((regionalCeiling - price) / regionalCeiling) * 100 : 0;
...
exports.computeValuation = computeValuation;
